...
//		toDb.setTaskFollowers(taskFollowers);
	...
	public ResponseEntity<ApiResponse> updateTask(Task taskfromClient) {
		if (!taskRepository.existsById(taskfromClient.getId())) {
			return new ResponseEntity<>(
					new ApiResponse(false, "Task with id : " + taskfromClient.getId() + " not found", null),
	...
	public void _processTaskStatus(Task taskfromDb, Task taskfromClient, UserDetails loggedInUser) {
		...
		if (ts.getConstByName().equals("COMPLETED")) {
			if (taskfromDb.getAssignee().getId() == loggedInUser.getId()) {
				_verifyTask(taskfromDb, taskfromClient, loggedInUser);
			}
		} else {
			taskfromDb.setCurrentStatus(ts);
			TaskComment comment = new TaskComment();
			comment.setComment("Task Moved to " + ts.getLabel() + " status on "
					+ format.format(new Date(System.currentTimeMillis())) + " by " + loggedInUser.getFirstName());
			comment.setDateTime(System.currentTimeMillis());
			comment.setUserDetails(loggedInUser);
			taskfromDb.getTaskComments().add(0, comment);
	...
	public void _verifyTask(Task taskfromDb, Task taskfromClient, UserDetails loggedInUser) {
		...
		TaskComment comment = new TaskComment();
		comment.setComment("Task has been verified on " + format.format(new Date(System.currentTimeMillis())) + " by "
	...
	public ResponseEntity<ApiResponse> updateTaskComment(TaskComment commentsFrmClient) {
		TaskComment commentFrmDb = taskCommentRepository.getOne(commentsFrmClient.getId());

		commentFrmDb.setComment(commentsFrmClient.getComment());
		commentFrmDb.setDateTime(System.currentTimeMillis());
		commentFrmDb.setUserDetails(currentUser.getCurrentUser());
		URI uri = MvcUriComponentsBuilder.fromController(TaskController.class).path("/{id}")
				.buildAndExpand(commentFrmDb.getId()).toUri();
		return ResponseEntity.created(uri).body(new ApiResponse(true, "", commentFrmDb));

	}

	public ResponseEntity<ApiResponse> getTaskById(Long id) {
		if(!taskRepository.existsById(id)) {
			return new ResponseEntity<ApiResponse>(new ApiResponse(false, "Task with id : "+id+" not found", null),
					HttpStatus.BAD_REQUEST);
		}
		Task fromDb = taskRepository.getOne(id);
		URI uri = ServletUriComponentsBuilder.fromCurrentRequest().buildAndExpand(fromDb.getId()).toUri();
		return ResponseEntity.created(uri).body(new ApiResponse(true, "", new TaskResource(fromDb)));
	}
